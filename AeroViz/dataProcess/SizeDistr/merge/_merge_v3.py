...
def _merge_data(_smps_ori, _aps_ori, _shift_ori, _smps_lb, _aps_hb, _shift_mode, _alg_type):
    ...
    # 3-way merge index: timestamps present in SMPS, APS *and* a valid (non-NaN)
    # shift — so QC-rejected (NaN-shift) timestamps are excluded, matching v1/v2/v4.
    # (Previously v3 used a 2-way smps∩aps index, which wrongly emitted SMPS-only
    #  "merged" rows for QC-rejected timestamps.)
